...
import { PresentationDialog } from "@/features/lessons/PresentationDialog";
...
export default function LessonEditorPage() {
  ...
  const { currentLesson, setCurrentLesson, activeTab, setActiveTab } =
    useLessonStore();
  ...
  const fetchLesson = async () => {
    try {
      ...
      if (content.materialsApproved) {
        ...
        const materials =
          parsedMaterials.educational_materials || parsedMaterials;
      ...
      if (
        content.testsApproved &&
        Array.isArray(content.testsApproved) &&
        content.testsApproved.length > 0
      ) {
      ...
      if (
        ...
        Array.isArray(rawLesson.quizQuestions) &&
        rawLesson.quizQuestions.length > 0
      ) {
        if (!lessonData.tests) {
